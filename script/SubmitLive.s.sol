...
/// Bundler-role submission of a pre-signed UserOp JSON via handleOps.
/// Env: OP_JSON, PRIVATE_KEY (bundler key), ENTRYPOINT, BENEFICIARY (optional)
contract SubmitLive is Script {
    function run() external {
        ...
        address payable beneficiary =
            payable(vm.envOr("BENEFICIARY", 0x90F79bf6EB2c4f870365E785982E1f101E93b906));

        uint256 bk = vm.envUint("PRIVATE_KEY");
        vm.startBroadcast(bk);
        IEntryPoint(vm.envAddress("ENTRYPOINT")).handleOps(ops, beneficiary);
        vm.stopBroadcast();
        console2.log("handleOps submitted");
    }
}
